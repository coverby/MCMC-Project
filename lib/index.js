...
function main() {
    var graph = [];
    const filename = rl.question('Please specify input file (in current dir): ');
    ...
    if ( !(connected(graph)) ) throw new Error('Input graph is not connected! Aborting...');
    const iterationcount = parseInt(rl.question('Please specify the number of graph iterations to attempt: '));
    
    console.log('Beginning iterations...');
    const result = looper(graph, iterationcount);
    console.log('Finished!');
    if (rl.keyInYN('Do you want a report in the terminal?')) {
        console.log('Example most likely graph (edges):');
        for (let i = 0; i < result.topg[0].edges.length; i++) {
            console.log(result.topg[0].edges[i]);
        }
        console.log(`Expected number of edges from node 0 in top 1%: ${result.avg0.toFixed(3)}`);
        console.log(`Expected number of total edges in top 1%: ${result.avge.toFixed(3)}`);
        console.log(`Expected maximum optimal path length in top 1%: ${result.avgw.toFixed(3)}`);
    }
}
...
function looper(graph, iterationcount) {
    ...
    var topgraphcount = Math.ceil(iterationcount*.01); // # members of top 1% of graphs
    /* istanbul ignore if */
    if ( graph.edges.length >= ( (graph.nodes**2 ) - graph.nodes)/ 2 ) {
        topgraphcount = ( (graph.nodes**2 ) - graph.nodes)/ 2 - 1;
    }
    ...
    while (updates < iterationcount - 1) {
        ...
        // This progress bar didn't work as intended
    ...
    // Now we have a problem: the most probable individual graphs might represent over 1% of population
    // So we need to rescale to the actual counts we found
    var newcount = 0;
    var newindex = 0;
    for (let i = 0, n = sortedcounts.length; i < n; i++) {
        //console.log(`logging i: ${i}, newcount: ${newcount}`);
        newcount += sortedcounts[i];
        /* istanbul ignore next */
        if (newcount >= topgraphcount) { //This just refuses to be covered too
            newindex = i;
            break;
        }
    }
    ...
    var avgedges = 0;
    var avgworst = 0;
    for (let i = 0, n = newindex + 1; i < n; i++) {
        //console.log(`loop at i: ${i}, graph: ${mostprobgraphs[i]}`);
        mostprobgraphs[i].genadjacencylist();
        avgedgesfrom0 += sortedcounts[i]*mostprobgraphs[i].adjlist[0].length;
        avgedges += sortedcounts[i]*mostprobgraphs[i].edges.length;
        avgworst += sortedcounts[i]*mostprobgraphs[i].longestoptpath;
    }
    avgedgesfrom0 = avgedgesfrom0/newcount;
    avgedges = avgedges/newcount;
    avgworst = avgworst/newcount;
    return {
