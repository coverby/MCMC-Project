...
class Graph {
    ...
    hash() {
        ...
        var hash = 5381;
        for (let i = 0, n = this.edgelist.length; i < n; i++) {
            for (let j = 0, q = this.edgelist[i].length; j < q; j++) {
                hash += (hash*33 + this.edgelist[i][j]) % 9007199254740991; // keep in double int range
            }
        }
        this.hashval = hash;
    }
