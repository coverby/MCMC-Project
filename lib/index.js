...
function edgeproposal(graph) {
    ...
    if ( graph.edgelist.includes(null) ) graph.genedgelist;
    var canadd = true;
    var cansub = true;
    var tempedgelist = Array(graph.nodes);
    var newgraph = graph;
    for (let i = 0; i < graph.nodes; i++) tempedgelist[i] = i; // find better method
    if ( graph.edgelist.length <= 1 ) cansub = false;
    if ( graph.edgelist.length >= graph.nodes*graph.nodes*(graph.nodes - 1) ) canadd = false; 
    const thetaold = graph.Gamma*edgeweight(graph) + nodecrawler(graph);
    const deciderand = Math.random();
    const rand = Math.random();
    if (canadd && deciderand >= .5) {
        // eslint-disable-next-line no-console
        console.log('entered add path');
        const node1 = Math.round(Math.random()*graph.nodes) - 1;
        tempedgelist.splice(node1, 1);
        const node2 = tempedgelist[Math.round(Math.random()*(graph.nodes - 1)) - 1];
        newgraph.connect(node1, node2);
        const thetanew = newgraph.Gamma*edgeweight(newgraph) + nodecrawler(newgraph);
        if (Math.min(1, Math.exp(-(thetanew - thetaold)/graph.Temp)) >= rand) return newgraph;
        // eslint-disable-next-line no-console
        console.log('add failed');
    }
    if (cansub && deciderand < .5) {
        // eslint-disable-next-line no-console
        console.log('entered subtract path');
        const edge1 = Math.round(Math.random()*graph.edgelist.length);
        newgraph.remove(edge1);
        const thetanew = newgraph.Gamma*edgeweight(newgraph) + nodecrawler(newgraph);
        if (Math.min(1, Math.exp(-(thetanew - thetaold)/graph.Temp)) >= rand && newgraph.connected()) return newgraph;
        // eslint-disable-next-line no-console
