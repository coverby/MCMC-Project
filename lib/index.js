    ...
    hash() {
        ...
        for (let i = 0, n = this.adjlist.length; i < n; i++) {
            for (let j = 0, q = this.adjlist[i].length; j < q; j++) {
                // console.log(`hash call on edgelist: ${this.edgelist[i]} index: ${j}`);
...
function samegraph(graph1, graph2) {
    if (graph1.edges.length === graph2.edges.length) {
        for (let i = 0; i < graph1.nodes; i++) {
            for (let j = 0; j < graph2.edgelist[i].length; j++) {
                if (!(graph1.edgelist[i].includes(graph2.edgelist[i][j]))) return false;
            }
        }
    }
    return true;
...
function looper(graph, iterationcount) {
    ...
    while (updates < iterationcount - 1) {
        ...
        if (newgraph.updatecount > updates) {
            ...
            if (newgraph.weight < topgraphweights[topgraphcount - 1]) {
                ...
                // Binary search below would be faster but needs extra tracking here
                    for (let i = topgraphcount - 2; i < 1; i--) {
                        if ( newgraph.weight > topgraphweights[i] ) { //We already decided it belongs in top %
                            index = i + 1;
                ...
                // console.log(`count added: ${probgraphs.count[lastgraphindex]}`);
            }
            graph = _.cloneDeep(newgraph);
        }
        if (Math.floor(graph.updates*barcount/iterationcount) > barprog) {
            bar.tick();
            barprog += 1;
        }
        sanitycount += 1;
    ...
    //console.log(`Iterations finished with ${sanitycount} loops!`);
    //console.log(`top graphs: ${mostprobgraphs}`);
    //console.log(`top counts: ${sortedcounts}`);
    //console.log(`maxcount: ${Math.max(...probgraphs.count)}`);
    return {
...
function edgeproposal(graph) {
    ...
    while (!exit) {
        ...
        if ((canadd && deciderand >= .5) || !cansub) {
            ...
            //console.log(`add attempt with n1 ${node1} and n2 ${node2} (index: ${tempindex}), graph # edges: ${newgraph.edges.length}, temp edges: ${tempedges}, tempelength: ${tempedges.length}`);
            //console.log(`old graph: ${graph.edges}`);
            //console.log(`new graph: ${newgraph.edges}`);
...
module.exports = { Graph, connected, edgeweight, nodecrawler, readfile, edgeproposal, main, looper, samegraph };
