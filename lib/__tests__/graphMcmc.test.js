...
describe('graph statistics analysis', () => {
    it('should correctly compare graphs', () => {
        ...
        graph3.Temp = 10;
        graph3.genedgelist();
        const resultsame = graphMcmc.samegraph(graph, graph2); // equal
        const resultdiff = !graphMcmc.samegraph(graph, graph3); // not equal
        // eslint-disable-next-line no-console
        // console.log(result);
        assert(resultsame, 'identical graphs not reported as such');
        assert(resultdiff, 'different graphs not reported as such');
    });
    ...
    it('should return top 1% of graphs', () => {
        var coords = [[0, 0], [0, 1], [1, 0], [2, 5], [6, 1], [-2, 4]];
        const graph = new graphMcmc.Graph(6, coords);
        graph.connect(0, 1);
        graph.connect(0, 2);
        graph.connect(1, 2);
        ...
        const result = (
            output.topw.length === 5 &&
            output.topg.length === 5 &&
            output.probg.length === 5 &&
            output.probcounts.length === 5 &&
            !output.topw.includes(Infinity) &&
            !output.probcounts.includes(NaN)
        );
        //console.log(`# weights: ${output.w.length}, # graphs: ${output.g.length}, finite values: ${!output.w.includes(Infinity)}`);
        //console.log(`weights: ${output.w}`);
        assert(result, 'top 1% has unexpected parameters');
    });
